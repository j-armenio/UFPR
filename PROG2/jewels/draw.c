    ...
    for (i = 0; i < MATRIX_SIZE; i++){
        for (j = 0; j < MATRIX_SIZE; j++){
            if (gm->matrix[i][j]->fishType == FISH_TYPE_NULL)
                ...
                al_draw_bitmap(gm->matrix[i][j]->sprite[1], gm->matrix[i][j]->fishX, gm->matrix[i][j]->fishY, 0);
            } else {
                al_draw_bitmap(gm->matrix[i][j]->sprite[0], gm->matrix[i][j]->fishX, gm->matrix[i][j]->fishY, 0);
            }
